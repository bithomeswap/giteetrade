...
class TraderSpi(traderapi.CTORATstpTraderSpi):
    ...
    def OnRspUserLogin(self, pRspUserLoginField: "traderapi.CTORATstpRspUserLoginField", pRspInfoField: "CTORATstpRspInfoField", nRequestID: "int") -> "void":
        if pRspInfoField.ErrorID == 0:
            ...
            #【property是继承了一个类】这里是传递构建了一个参数类
            #先把账户密码、投资者信息、登录信息都查询完才能查询ETF清单，不然可能会报错
            if 1:
                # 查询ETF清单信息
            ...
            if 1:
                # 查询ETF成份证券信息
                print("查询 CTORATstpQryETFBasketField")
            ...
            if 1:
                # 查询投资者限仓
                req_field = traderapi.CTORATstpQryInvestorPositionLimitField()
                # 以下字段不填表示不设过滤条件，即查询所有etf
                # req_field.ExchangeID = traderapi.TORA_TSTP_EXD_SSE
                print("查询投资者限仓")
                self.__req_id += 1
                ret = self.__api.ReqQryInvestorPositionLimit(req_field, self.__req_id)
                if ret != 0:
                    print('ReqQryInvestorPositionLimit fail, ret[%d]' % ret)
            if 0:
                # 查询资金账号
    ...
    def OnRspQryETFFile(self, pETFFileField: traderapi.CTORATstpETFFileField, pRspInfoField: traderapi.CTORATstpRspInfoField, nRequestID: int, bIsLast: bool) -> "void":
        allETFFile.append(pETFFileField.dict())
        ETFFiledf=pd.DataFrame(allETFFile)
        print(len(ETFFiledf))
        ETFFiledf.to_csv("ETFFiledf.csv")
        # pass
    #获取ETF成分券
    def OnRspQryETFBasket(self, pETFBasketField: traderapi.CTORATstpETFBasketField, pRspInfoField: traderapi.CTORATstpRspInfoField, nRequestID: int, bIsLast: bool) -> "void":
        allETFBasketField.append(pETFBasketField.dict())
        ETFBasketFielddf=pd.DataFrame(allETFBasketField)
        print(len(ETFBasketFielddf))
        ETFBasketFielddf.to_csv("ETFBasketFielddf.csv")
        # pass

    # #查询投资者限仓信息【回调函数只有一个】
    # def OnRspQryInvestorPositionLimit(self, pQryInvestorPositionLimitField: traderapi.CTORATstpQryInvestorPositionLimitField, pRspInfoField: traderapi.CTORATstpRspInfoField, nRequestID: int, bIsLast: bool) -> "void":
    #     print("pQryInvestorPositionLimitField",pQryInvestorPositionLimitField)#返回空值【可能是模拟盘的问题】
