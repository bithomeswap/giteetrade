...
# # filepath=Path(rf"{basepath}\20240624195327_41.pdf")
...
# # #【批量文件转CSV】用opencv处理过之后的数据，识别准确率有所降低，可能是不如原始图片清晰
# alldf=pd.DataFrame({})
# import os
# from pathlib import Path
# # 指定要上传文件的文件夹路径
# folder_path = rf"{basepath}\一分一档表"
...
# # #PDF文件处理【被每一页的标题搞乱套了】没啥用
# from pathlib import Path
# # filepath=Path(rf"{basepath}\20240722163024_223.xlsx")
# filepath=Path(rf"{basepath}\20240624195327_41.pdf")
# file_object = client.files.create(file=filepath,purpose="file-extract")
# # 获取文件读取结果
# # file_content = client.files.retrieve_content(file_id=file_object.id)#旧版本retrieve_content
...
# print("file_content",file_content)
# # 将字符串转换为CSV
# row = file_content.strip().split(r'\n')#格式其实是字符串列表，这里取第一个
# print("row",row)
# # cells = row.split('|')[1:-1] # 删掉第一行和最后一行
# # print(cells,type(cells))#这个是列表
# # #数据处理【逐行拼接df并生成csv文件】
