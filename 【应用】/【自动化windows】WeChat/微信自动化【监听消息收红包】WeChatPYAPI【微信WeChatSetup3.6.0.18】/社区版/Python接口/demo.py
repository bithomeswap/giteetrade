...
# 需要在指定文件夹当中执行才能正常调用这个pyd文件
from WeChatPYAPI import WeChatPYApi#在其他地方执行会标记报错，但是不影响文件执行，因为是在调用一个pyd文件
...
def main():
    ...
    # 公众号列表：pull_type = 3
    # 其他：pull_type = 4
    lists = w.pull_list(pull_type=3)#拉取各种列表【目前拉取的数据不够需要等一段时间】
    time.sleep(5)
    print(lists)
